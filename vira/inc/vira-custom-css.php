...
if(!function_exists("general_selector")){
...
if(!function_exists("general2_selector")){
function general2_selector(){
  return array(
    'background-color' => '.misha_loadmore,.mini_cart_counter,.main-cart .woocommerce-mini-cart__buttons.buttons a.checkout,.tabs-panel-mobile .add_comment_mobile,.woocommerce-mini-cart__buttons.buttons a:nth-child(2),.form_search.active::after,.account_mobile .loged,.slider-right .owl-dots .active,.woocommerce #respond .form-submit #submit,.percent-option-rating strong,.checkout-headers ul .nav span.actv,.checkout-headers.cart ul li.bar:nth-child(2) span,.checkout-headers.checkout ul li.bar:nth-child(2) span,.checkout-headers.checkout ul li.bar:nth-child(4) span,.checkout-headers.thankyou ul li.bar:nth-child(4) span ,.checkout-headers.thankyou ul li.bar:nth-child(2) span,.summary .cart-pro .tab-pro ul li::after,.ui-slider .ui-slider-range,.index-product .woocommerce-pagination ul .page-numbers.head-side,.item-thumb-index .cat-name-index',
...
if(!function_exists("links_selector")){
...
function prk_custom_font() {
	// اگر گزینه فونت سفارشی فعال نباشه، هیچی برنگردون
	if (!prk_option('custom_font')) {
		return '';
	}

	$font_name = trim(prk_option('theme_font_name'));
	if (!$font_name) {
		$font_name = 'prk-font'; // پیش‌فرض
	}

	$font_css = '';

	// 🔹 وزن نرمال (400)
	$font_css .= "@font-face{font-family:'{$font_name}';font-display:swap;font-fallback:arial,sans-serif,tahoma;font-weight:400;src:";

	if (!empty(prk_option('theme_font_woff2'))) {
		$font_css .= "url('" . prk_option('theme_font_woff2') . "') format('woff2'),";
	}
	if (!empty(prk_option('theme_font_woff'))) {
		$font_css .= "url('" . prk_option('theme_font_woff') . "') format('woff'),";
	}
	if (!empty(prk_option('theme_font_ttf'))) {
		$font_css .= "url('" . prk_option('theme_font_ttf') . "') format('truetype'),";
	}
	if (!empty(prk_option('theme_font_eot'))) {
		$font_css .= "url('" . prk_option('theme_font_eot') . "') format('embedded-opentype'),";
	}
	if (!empty(prk_option('theme_font_svg'))) {
		$font_css .= "url('" . prk_option('theme_font_svg') . "') format('svg')";
	}

	$font_css = rtrim($font_css, ','); // حذف ویرگول اضافی
	$font_css .= ';}';

	// 🔹 وزن بولد (500 یا بالاتر)
	$font_css .= "@font-face{font-family:'{$font_name}';font-display:swap;font-fallback:arial,sans-serif,tahoma;font-weight:500;src:";

	if (prk_option('custom_bold_font') && !empty(prk_option('theme_font_bold_woff2'))) {
		$font_css .= "url('" . prk_option('theme_font_bold_woff2') . "') format('woff2'),";
	} elseif (!empty(prk_option('theme_font_woff2'))) {
		$font_css .= "url('" . prk_option('theme_font_woff2') . "') format('woff2'),";
	}

	if (prk_option('custom_bold_font') && !empty(prk_option('theme_font_bold_woff'))) {
		$font_css .= "url('" . prk_option('theme_font_bold_woff') . "') format('woff'),";
	} elseif (!empty(prk_option('theme_font_woff'))) {
		$font_css .= "url('" . prk_option('theme_font_woff') . "') format('woff'),";
	}

	if (prk_option('custom_bold_font') && !empty(prk_option('theme_font_bold_ttf'))) {
		$font_css .= "url('" . prk_option('theme_font_bold_ttf') . "') format('truetype'),";
	} elseif (!empty(prk_option('theme_font_ttf'))) {
		$font_css .= "url('" . prk_option('theme_font_ttf') . "') format('truetype'),";
	}

	if (prk_option('custom_bold_font') && !empty(prk_option('theme_font_bold_eot'))) {
		$font_css .= "url('" . prk_option('theme_font_bold_eot') . "') format('embedded-opentype'),";
	} elseif (!empty(prk_option('theme_font_eot'))) {
		$font_css .= "url('" . prk_option('theme_font_eot') . "') format('embedded-opentype'),";
	}

	if (prk_option('custom_bold_font') && !empty(prk_option('theme_font_bold_svg'))) {
		$font_css .= "url('" . prk_option('theme_font_bold_svg') . "') format('svg')";
	} elseif (!empty(prk_option('theme_font_svg'))) {
		$font_css .= "url('" . prk_option('theme_font_svg') . "') format('svg')";
	}
	...
	// 🔹 اعمال روی body و html
	$font_css .= "body,html{font-family:'{$font_name}',Arial,sans-serif,Tahoma !important;font-size:15px !important;}";

	return $font_css;
}


add_action('wp_head', 'prk_output_custom_font_style', 6);
function prk_output_custom_font_style() {
    $css = prk_custom_font();
    if (!empty($css)) {
...
add_action('wp_footer', 'add_script_foot',0);
