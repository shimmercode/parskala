...
class Ajax_Controller {
    ...
    public function handle_send_otp() {
        ...
        $lock_key = 'vira_otp_lock_' . md5( $mobile . ( $_SERVER['REMOTE_ADDR'] ?? '' ) );
        ...
        $sent = function_exists( 'vira_send_sms' ) ? vira_send_sms( $mobile, 'کد ورود شما به ویرا: ' . $code ) : false;
        if ( ! $sent ) {
            wp_send_json_error( array( 'message' => 'ارسال پیامک پیکربندی نشده است. کلید API را در مرکز فرماندهی وارد کنید.' ) );
        }
