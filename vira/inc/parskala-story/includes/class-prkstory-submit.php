...
class Prkstory_Submit {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_prkstory_do_submit_form_user_single', array( $this, 'submit_handler_user_single' ) );
		...
		// Ajax file upload.
		add_action( 'wp_ajax_prkstory_do_ajax_file_upload', array( $this, 'ajax_upload' ) );
		add_action( 'wp_ajax_nopriv_prkstory_do_ajax_file_upload', array( $this, 'ajax_upload' ) );
	}
	...
	public function public_story_fields() {
		...
		if ( $parents_query->have_posts() ) :
			?>
			<div class="prkstory-form-row prkstory-current-category-wrapper">
				<label for="prkstory-story-parent">
					<span><?php esc_html_e( 'Story Category', 'prk-story' ); ?></span>
					<button class="prkstory-inline-button prkstory-parent-open-new" type="button">
						<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000">
							<path d="M0 0h24v24H0V0z" fill="none" />
							<path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" />
						</svg>
						<?php esc_html_e( 'create new', 'prk-story' ); ?>
					</button>
					<select class="prkstory-input" name="prkstory-story-parent" id="prkstory-story-parent">
						<?php
		...
		<?php endif; ?>
		<div class="prkstory-new-category-wrapper<?php echo $parents_query->have_posts() ? ' prkstory-new-category-wrapper--has-parent' : ''; ?>">
			<?php if ( $parents_query->have_posts() ) : ?>
				<button class="prkstory-inline-button prkstory-parent-open-current" type="button">
					<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000">
						<path d="M0 0h24v24H0V0z" fill="none" />
						<path d="M21 11H6.83l3.58-3.59L9 6l-6 6 6 6 1.41-1.41L6.83 13H21v-2z" />
					</svg>
					<?php esc_html_e( 'select category', 'prk-story' ); ?>
				</button>
			<?php endif; ?>
			<div class="prkstory-form-row prkstory-form-row-center">
				<label>
					<span><?php esc_html_e( 'Story Category Image', 'prk-story' ); ?></span>
				</label>
				<input type="file" class="prkstory-story-media prkstory-story-media--parent">
				<input type="hidden" class="prkstory-story-media-id" data-required="false" name="prkstory-story-media-id"<?php disabled( $parents_query->have_posts() ); ?>>
			...
			<div class="prkstory-form-row">
				<label for="prkstory-story-circle-title">
					<span><?php esc_html_e( 'Story Category Title', 'prk-story' ); ?></span>
					<input class="prkstory-input prkstory-link-text" type="text" name="prkstory-story-circle-title" id="prkstory-story-circle-title"<?php disabled( $parents_query->have_posts() ); ?>>
				</label>
			</div>
		</div>
		<?php
		...
		return '<div class="prkstory-submit-item">' . ob_get_clean() . '</div>';
	}
	/**
	 * Get form html.
	 *
	...
	public function form_content( $type = '' ) {
		...
		<div class="prkstory-submit-form-wrapper">
			<form class="prkstory-submit-form" enctype="multipart/form-data" data-type="<?php echo esc_attr( $type ); ?>">
				<?php echo 'user-public' === $type ? $this->public_story_fields() : null; // phpcs:ignore WordPress.Security.EscapeOutput ?>
				<div class="prkstory-submit-item">
					<?php if ( prkstory_helpers()->options( 'allow_link', true ) ) : ?>
						<div class="prkstory-form-row">
							<label>
								<span><?php esc_html_e( 'Story Link Text', 'prk-story' ); ?></span>
								<input class="prkstory-input prkstory-link-text" type="text" name="prkstory-story-link-text">
							</label>
					...
					<div class="prkstory-form-row prkstory-edit-wrapper">
						<label>
							<span><?php esc_html_e( 'Story Media', 'prk-story' ); ?></span>
							<span class="prkstory-need-text"><?php esc_html_e( 'Upload File', 'prk-story' ); ?></span>
						</label>
						<?php
						$allowed_types      = prkstory_helpers()->options( 'allowed_types' );
						...
						?>
						<input type="file" class="prkstory-story-media prkstory-story-media--attachment">
						<input type="hidden" class="prkstory-story-media-id" data-required="true" name="prkstory-story-item-media-id">
						<?php if ( ! empty( $allowed_types ) ) : ?>
							<span class="prkstory-form-description"><small><?php printf( /* translators: %s: file types */ esc_html__( 'Allowed types: %s.' ), esc_attr( $allowed_types_str ) ); ?></small></span>
						<?php endif; ?>
	...
	public function submit_handler_user_public() {
		...
		do_action( 'prkstory_before_story_submit', 'public' );

		$user             = wp_get_current_user();
		$user_story_count = prkstory_helpers()->user_story_count( $user->ID, 'prkstory-public' );
		$story_limit      = prkstory_helpers()->options( 'user_public_story_limit', 10 );
		...
		if ( isset( $_POST['prkstory-story-parent'] ) ) {
			$parent_id        = (int) wp_unslash( $_POST['prkstory-story-parent'] );
			$user_item_count  = (array) prkstory_helpers()->user_story_item_count( $user->ID, $parent_id );
			$story_item_limit = prkstory_helpers()->options( 'user_public_story_item_limit', 10 );
			...
			if ( ! prkstory_helpers()->user_can_manage_story( $user->ID, $parent_id ) ) {
			...
			$parent_id    = null;
			$parent_title = isset( $_POST['prkstory-story-circle-title'] ) ? sanitize_text_field( wp_unslash( $_POST['prkstory-story-circle-title'] ) ) : '';
			$parent_thumb = isset( $_POST['prkstory-story-media-id'] ) && ! empty( $_POST['prkstory-story-media-id'] ) ? wp_unslash( (int) $_POST['prkstory-story-media-id'] ) : null;
		}

		$attachment_id = isset( $_POST['prkstory-story-item-media-id'] ) && ! empty( $_POST['prkstory-story-item-media-id'] ) ? wp_unslash( (int) $_POST['prkstory-story-item-media-id'] ) : null;
		...
		if ( ! $parent_id ) {
			...
			if ( is_wp_error( $parent_id ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Story can not be published. Try again later.', 'prk-story' ) ) );
			}
		...
		$link_text = isset( $_POST['prkstory-story-link-text'] ) && ! empty( $_POST['prkstory-story-link-text'] ) ? sanitize_text_field( wp_unslash( $_POST['prkstory-story-link-text'] ) ) : '';
		$link      = isset( $_POST['prkstory-story-link'] ) && ! empty( $_POST['prkstory-story-link'] ) ? esc_url_raw( wp_unslash( $_POST['prkstory-story-link'] ) ) : '';
		$duration  = isset( $_POST['prkstory-story-duration'] ) && ! empty( $_POST['prkstory-story-duration'] ) ? wp_unslash( (int) $_POST['prkstory-story-duration'] ) : '';
		$status    = prkstory_helpers()->options( 'user_publish_status', 'draft' );

		if ( ! prkstory_helpers()->options( 'allow_link', true ) ) {
			$link_text = '';
			$link      = '';
		}
		...
		$post_object = array(
			'text'     => $link_text,
			'link'     => $link,
			'duration' => $duration > 0 ? $duration : prkstory_helpers()->options( 'default_story_duration', 3 ),
			'image'    => array(
		...
		$story_id = wp_insert_post(
			array(
				'post_author' => $user->ID,
				'post_title'  => prkstory_helpers()->get_user_name( $user->ID ),
				'post_type'   => 'prkstory-public',
				'post_status' => $status,
	...
	public function submit_handler_user_single() {
		...
		do_action( 'prkstory_before_story_submit', 'single' );

		$user             = wp_get_current_user();
		$user_story_count = prkstory_helpers()->user_story_count( $user->ID );
		$story_limit      = prkstory_helpers()->options( 'user_single_story_limit', 10 );
		...
		update_user_meta( $user->ID, 'prkstory_last_updated', current_time( 'mysql' ) );
		wp_send_json_success( array( 'message' => $published_message ) );
	}
	...
	public function ajax_item_delete() {
		...
		if ( 'prkstory-user' !== get_post_type( $box_id ) ) {
	...
	public function ajax_upload() {
		...
		$allowed_types   = prkstory_helpers()->get_allowed_file_types( 'array' );
