...
/**
 * Remove only the visual loading overlay used while the ext nodes are being
 * loaded for the first click. Unlike mini_cart_ext_unblock(), this must not
 * close the active node.
 *
 * @param {JQuery Object} $node
 */
var mini_cart_ext_unblock_loading = function($node) {
    $node.removeClass('processing').unblock();
    $('body').trigger('nasa_publish_coupon_refresh');
};
...
var mini_cart_ext_url = function(endpoint) {
    ...
    if (typeof woocommerce_params !== 'undefined' && typeof woocommerce_params.wc_ajax_url !== 'undefined') {
        wc_ajax_url = woocommerce_params.wc_ajax_url.toString();
    }
    else if (typeof wc_cart_fragments_params !== 'undefined' && typeof wc_cart_fragments_params.wc_ajax_url !== 'undefined') {
        wc_ajax_url = wc_cart_fragments_params.wc_ajax_url.toString();
    }
    else if (typeof wc_add_to_cart_params !== 'undefined' && typeof wc_add_to_cart_params.wc_ajax_url !== 'undefined') {
        wc_ajax_url = wc_add_to_cart_params.wc_ajax_url.toString();
    }
    else {
        wc_ajax_url = window.location.href.split('#')[0].split('?')[0] + '?wc-ajax=%%endpoint%%';
    }

    return wc_ajax_url.replace('%%endpoint%%', endpoint);
};
...
/**
 * Prepare ext mini cart wrapper
 */
var mini_cart_ext_prepare = function($cart) {
    if (!$('body').hasClass('canvas-on')) {
        $('body').addClass('canvas-on');
    }

    if (!$cart.hasClass('ext-loading')) {
        $cart.addClass('ext-loading');
    }

    if ($cart.find('.ext-nodes-wrap').length < 1) {
        $cart.append('<div class="ext-nodes-wrap"></div>');
    }

    if ($cart.find('.ext-nodes-wrap .close-nodes').length < 1) {
        $cart.find('.ext-nodes-wrap').prepend('<a href="javascript:void(0);" class="close-nodes"></a>');
    }
};

/**
 * Refresh WooCommerce country/state fields after shipping panel opens.
 */
var mini_cart_ext_refresh_shipping = function($cart) {
    setTimeout(function() {
        $cart.find('select.country_to_state, input.country_to_state').trigger('change');
        $(document.body).trigger('country_to_state_changed');
    }, 100);
};

/**
 * Open requested ext mini cart node.
 */
var mini_cart_ext_open_node = function($cart, act) {
    if (!$cart.length || !act) {
        return;
    }

    mini_cart_ext_prepare($cart);

    $cart.find('.ext-nodes-wrap .ext-node').removeClass('active');

    var $node = $cart.find('.ext-nodes-wrap .ext-node.mini-cart-' + act);

    if ($node.length) {
        $node.addClass('active');

        if (act === 'shipping') {
            mini_cart_ext_refresh_shipping($cart);
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data && typeof data.content !== 'undefined') {
                $cart.find('.ext-nodes-wrap').append(data.content);

                $cart.find('.ext-nodes-wrap .ext-node').removeClass('active');
                $cart.find('.ext-nodes-wrap .ext-node.mini-cart-' + act).addClass('active');
        ...
        error: function () {
            $cart.removeClass('ext-loading');
        }
    });
};

/**
 * Open pending node after initial async load.
 */
var mini_cart_ext_open_pending = function($cart, animateDelay) {
    var act = $cart.data('pending-ext-mini-cart-act');

    if (act) {
        $cart.removeData('pending-ext-mini-cart-act');

        if (animateDelay) {
            setTimeout(function() {
                mini_cart_ext_open_node($cart, act);
            }, 120);
        }
        else {
            mini_cart_ext_open_node($cart, act);
 ...
 * Open ext mini cart
 */
$('body').on('click', '.ext-mini-cart', function(e) {
    e.preventDefault();

    var $this = $(this),
        act = $this.attr('data-act'),
        $cart = $this.parents('.prk-static-sidebar');
    ...
    if ($cart.length < 1 || $cart.find('.ext-mini-cart-wrap').length < 1) {
        return false;
    }

    /*
     * On first sidebar open WooCommerce/Nasa loads all ext nodes asynchronously and
     * temporarily disables the buttons. Do not swallow the user's first click;
     * queue it and open the requested panel as soon as the initial load finishes.
     */
    if ($cart.find('.ext-mini-cart-wrap').hasClass('nasa-disable') || $cart.data('ext-mini-cart-loading-all')) {
        $cart.data('pending-ext-mini-cart-act', act);
        mini_cart_ext_prepare($cart);
        mini_cart_ext_block($cart);
        return false;
    }
...
$('body').on('nasa_load_ext_mini_cart', function() {
    ...
    if ($cart.find('.ext-nodes-wrap .ext-node').length < 1) {
        $.ajax({
            ...
            data: {},
            beforeSend: function() {
                $cart.data('ext-mini-cart-loading-all', true);
            ...
            success: function (data) {
                if (data && typeof data.content !== 'undefined') {
                    ...
                    if ($cart.find('select.country_to_state, input.country_to_state').length) {
                        mini_cart_ext_refresh_shipping($cart);
                    }
            ...
            complete: function() {
                $cart.removeData('ext-mini-cart-loading-all');

                if ($('.ext-mini-cart-wrap').length) {
                    $('.ext-mini-cart-wrap').removeClass('nasa-disable');
                }

                mini_cart_ext_unblock_loading($cart);
                mini_cart_ext_open_pending($cart, true);
            }
        });
    }
    else {
        ...
        mini_cart_ext_open_pending($cart, false);
    }
});
