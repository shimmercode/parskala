      ...
      if( $product->is_type('variable') && $prk_chartp  ) {
       ...
       foreach($product_childs as $product_child ){
         if ( get_post_meta($product_child,'date_and_price_chart',true) ) {
           echo '<li class="action"><a href="#" onclick="get_price_chart_product('.$product->get_id().');" data-remodal-target="modalchartprice">
            ...
            <span class="product-more-icon-text">'.__('Price chart','parskala').'</span>
           </a></li>';
           break;
         }
       }
         ...
         <?php if ( !prk_check_in_stock() || !prk_check_amazing() ): ?>
          <li class="action">
             <a <?= $data_target?>>
               ...
               <span class="product-more-icon-text"><?php esc_html_e('notif me','parskala');?></span>
             </a>
           </li>
         <?php endif; ?>
       <?php endif; ?>
       ...
       <?php
        if ($compare_page && $product_compare && $general_product_compare) {
         ...
         <li>
          ...
          <span class="product-more-icon-text"><?php esc_html_e('Product compare','parskala');?></span>
          </a>
         </li>
       <?php
       }
       ...
       <?php if ($product_share):?>
        <li class="action">
            ...
            <span class="product-more-icon-text"><?php esc_html_e('share to','parskala');?></span>
          </a>
        </li>
      <?php endif;?>
...
<?php if (prk_option('prk_modern_mobile') == 1 ): ?>
  ...
  <?php if ( !mobile_cheker() && !tablet_cheker() && empty('prk-fashion' == theme_style())): ?>
    <div class="btns-pro-slider">
      ...
      if ($product_whislist) {
        ...
        echo '<span class="tooltiptext">'.__('Add to favorites','parskala').'</span>';
        echo '</div>';
        }else{
     ...
     <?php if ($product_share):?>
        ...
        <span class="tooltiptext"><?php esc_html_e('share to','parskala');?></span></div>
      </a>
    <?php endif;?>
   ...
   <?php if (prk_option('show_wolfnotify') &&  !$product->is_type('grouped')): ?>
     <?php if ( !prk_check_in_stock() || !prk_check_amazing() ): ?>
       ...
       <a <?= $data_target?>>
         <div class="btns-pro">
           <i class="<?= $active_icon?> <?=$class_done?> btns"></i>
           <span class="tooltiptext"><?php esc_html_e('notif me','parskala');?></span>
         </div>
       </a>
   ...
   <?php endif; ?>
    ...
    <?php
     if ($compare_page && $product_compare && $general_product_compare) {
      ...
      <span class="tooltiptext"><?php esc_html_e('Product compare','parskala');?></span>
       </a>
      </div>
    <?php
    }
     ...
     foreach($product_childs as $product_child ){
       if ( get_post_meta($product_child,'date_and_price_chart',true) ) {
         echo '<div class="btns-pro"><span onclick="get_price_chart_product('.$product->get_id().');" class="product_details_icon chart_price" data-remodal-target="modalchartprice">
          ...
          <span class="tooltiptext">'.__('Price chart','parskala').'</span>
         </span></div>';
         break;
       }
     }
...
<?php elseif(empty('prk-fashion' == theme_style())):?>
