...
class THWVSF_Public {
	...
	private function enqueue_styles($suffix, $jquery_version) {
		...
		if($settings && is_array($settings)){
			...
			$global_settings   = isset($settings['swatch_global_settings']) ? $settings['swatch_global_settings'] : $settings;
			$design_type_css[] = $this->get_global_settings_css($global_settings);
			...
			if(isset($settings['swatch_design_default']) ){
				...
				foreach ($settings as $design_key => $design_values) {
					...
					if($design_key != 'swatch_global_settings'){
						...
						$design_type_css[] = $this->get_design_type_css($design_values, $design_key);
					}
				}
				...
				if($attr_design_mapping && is_array($attr_design_mapping)){
					...
					foreach ($default_designs as $design_key => $design_name) {
						...
						$design_type_css[]   = $this->get_design_type_css($settings, $design_key);
					}

				}else{

					$design_type_css[] = $this->get_design_type_css($settings,'swatch_design_default');
				}
			}
	...
	private function get_design_type_css($settings, $design_key){
	...
	private function get_global_settings_css($settings){
	...
	private function enqueue_scripts($suffix, $jquery_version) {
		...
		if($settings && is_array($settings)){
			...
			$clear_on_reselect        = THWVSF_Utils:: get_global_swatches_settings('clear_select');
			$behavior_of_out_of_stock = THWVSF_Utils:: get_global_swatches_settings('behavior_of_out_of_stock');
			//$show_item_on_label       = THWVSF_Utils:: get_global_swatches_settings('show_item_on_label');
		}
	...
	public function swatches_display($html, $args){
		...
		if(in_array($type, $swatch_types)){
			...
			$attr_type_html .= $this->swatch_display_options_html($html,$args,$type,$design_type);
	...
	public function swatch_display_options_html($html, $args, $type,$design_type){
		...
		if ( ! empty( $options ) ) {
			...
			if($product){
				...
				$default_label = wc_attribute_label($attribute);

				$html .= '<ul class="thwvsf-wrapper-ul"  data-default-label="'. esc_attr($default_label) .'">';
				...
				foreach ( $terms as $term ) {
	                ...
	                if($term_status){
		                ...
		                switch ($type) {
				            ...
				            break;
				            case 'label' : 
				            	$html .= $this->add_label_display($id,$name,$attribute,$term,$attr_class,$selected,$data_val,$tt_html,$design_class,$attr_method,$local_settings);
				            break;
		                }
		            }
	...
	public function add_label_display($id,$name,$attribute,$term,$attr_class,$selected,$data_val,$tt_html,$design_class,$attr_method,$local_settings){
		...
		if($attr_method == 'global'){
			...
			$label = $label ? $label : $name;
			$value = get_term_meta( $term->term_id,'product_'.$attribute, true );
			if(($auto_convert && $auto_convert == 'yes' && !$value) ||  (empty($value))){
				$value = $term->name;
			}
